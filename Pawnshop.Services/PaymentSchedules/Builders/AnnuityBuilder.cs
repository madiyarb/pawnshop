...
{
    public class AnnuityBuilder : BaseBuilder
    {
        ...
        public override List<ContractPaymentSchedule> Execute()
        {
            ...
            decimal monthPercent = _percentPerDay * 30;
            decimal monthlyPayment = (_loanCost / Convert.ToDecimal((1 - Math.Pow((double)(1 + (monthPercent / 100)), -_paymentsCount)) / ((double)monthPercent / 100)));
            ...
            for (; _counter < _paymentsCount; _counter++)
            {
                ...
                item.DebtCost = monthlyPayment * (decimal)Math.Pow((double)(1 + (monthPercent / 100)), (1 - (_paymentsCount - _counter) - 1));
