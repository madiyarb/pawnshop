...
{
    public class SendContactCommandHandler : IRequestHandler<SendContactCommand, bool>
    {
        private readonly IHttpSender _httpSender;
        private readonly IMediator _mediator;
        private const string _sendUrl = "hard/sendContacts";

        public SendContactCommandHandler(IHttpSender httpSender, IMediator mediator)
        {
            _httpSender = httpSender;
            _mediator = mediator;
        }

        public async Task<bool> Handle(SendContactCommand request, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    LogNotification = request.GetLogNotifications($"Отправка контактов по clientId={request.ClientId}")
                };
