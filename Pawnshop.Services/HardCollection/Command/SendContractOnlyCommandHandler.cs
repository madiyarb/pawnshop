...
{
    public class SendContractOnlyCommandHandler : IRequestHandler<SendContractOnlyCommand, bool>
    {
        ...
        public SendContractOnlyCommandHandler(IHttpSender httpSender, IMediator mediator)
        {
            _httpSender = httpSender;
            _mediator = mediator;
        }

        public async Task<bool> Handle(SendContractOnlyCommand request, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    request.ContractDataOnly = await _mediator.Send(new GetContractOnlyQuery() { ContractId = request.ContractId, IsJobWorking = request.IsJobWorking });
                }
                ...
                {
                    LogNotification = request.GetLogNotifications(@$"Отправлен частичные данные контракта в МП по ContractId={request.ContractId}
