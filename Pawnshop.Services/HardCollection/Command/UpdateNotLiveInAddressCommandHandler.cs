...
{
    public class UpdateNotLiveInAddressCommandHandler : IRequestHandler<UpdateNotLiveInAddressCommand, bool>
    {
        private readonly IMediator _mediator;

        public UpdateNotLiveInAddressCommandHandler(IMediator mediator)
        {
            _mediator = mediator;
        }

        public async Task<bool> Handle(UpdateNotLiveInAddressCommand request, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var notification = new HardCollectionNotification()
                {
                    ...
                    LogNotification = request.GetLogNotifications($"Не живет по адресу по ContractId={request.ContractId}")
                };
            ...
            catch (Exception ex) 
            {
                var notification = new HardCollectionNotification()
                {
                    LogNotification = request.GetLogNotifications($"Ошибка при сохранений не живет по адресу по ContractId={request.ContractId}", ex, Data.Models.Audit.EventStatus.Failed)
                };
                await _mediator.Publish(notification);
